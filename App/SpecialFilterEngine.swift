...
enum SpecialFilterEngine {
    ...
    /// БЫЛО 40 — баг: перебор идёт от СТРОГИХ подмножеств (threshold = 6, 5,
    /// 4...) к менее строгим, а именно на уровнях 5-6 из 6 реальных
    /// совпадений почти нет (мало у кого сразу 5-6 конкретных тегов), зато
    /// там уходит C(6,6)+C(6,5)+C(6,4)+C(6,3) = 1+6+15+20 = 42 запроса —
    /// бюджет в 40 кончался ДО того, как поиск доходил до уровней 1-2 из 6,
    /// где как раз и находится большинство реальных тайтлов (отсюда жалоба
    /// "выбрал 10 тегов — нашло всего 2 тайтла"). При maxItems = 6 полный
    /// перебор всех уровней — ровно 2^6 - 1 = 63 запроса, это и есть
    /// настоящий потолок (targetResultCount ниже — реальный ранний выход в
    /// обычном случае, когда совпадений и так уже достаточно).
    private static let maxRequests = 63
