...
struct DirectoryKind {
    ...
    // было). ВАЖНО (исправлено — раньше здесь было написано "сервер молча
    // проигнорирует неизвестное значение", это НЕВЕРНО): этот бэкенд строго
    // валидирует sort_by и отвечает 422 на неизвестное значение (см.
    // CatalogViewModel.fetchPage — "все проверено, rate/rating/votes/... —
    // 422"), поэтому неподтверждённая догадка здесь и правда МОГЛА бы
    // сломать список при выборе — но DirectoryListViewModel.fetchPage ловит
    // именно этот 422 и повторяет запрос вовсе без sort_by (серверный
    // порядок по умолчанию), так что худший случай — сортировка тихо не
    // применится, а не пустой экран с ошибкой.
